...
| Expose payload metrics only in benchmark surfaces | Record payload timing for a benchmarked layout run | PENDING | Await `benchmark-panel-ui.png` and `benchmark-run-context.json` | Must show all six payload metrics from `/benchmark` |
